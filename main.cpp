	...
	IDxcIncludeHandler* includeHandler)
{
	...
	// 1. hlslファイルを読む
	IDxcBlobEncoding* shaderSource = nullptr; // 修正: sourceBlob から shaderSource に統一
	HRESULT hr = dxcUtils->LoadFile(filePath.c_str(), nullptr, &shaderSource);
	// 読めなかったら止める
	assert(SUCCEEDED(hr));

	// 読み込んだファイルの内容を設定する
	DxcBuffer shaderSourceBuffer; // 修正: 先頭のSを小文字に統一
	...
	LPCWSTR arguments[] = {
		...
		L"-Zi", L"-Qembed_debug", // デバッグ用の情報を埋め込む (スライドに合わせて追加)
		L"-Od",           // 最適化を外しておく
		L"-Zpr",          // メモリレイアウトは行優先
	};
	...
	hr = dxcCompiler->Compile(
		...
		_countof(arguments),      // コンパイルオプションの数 (修正: _Countof から _countof へ)
...
#pragma region DirectX 12の初期化
	...
	*wvpDayta = MakeIdentity4x4();
	...
	// 各種行列の計算 (projectionMatrix の重複と MakeAffine の間違いを修正)
	...
	// 未定義変数だった部分を wvpDayta に修正してデータを転送
...
#pragma region コマンドキューを生成する
	...
	// RTV用のヒープでディスクリプタの数は二、RTVはShader内で触るものではないので、ShaderVisibleはfalse
...
#pragma endregion
	...
	//現時点でincludeはしないが、includに対応するための設定を行っておく
...
#pragma region PSO
	...
	// ※someStrideInBytes -> StrideInBytes に修正
...
#pragma endregion
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			// 行列を再計算してGPU上のバッファ(wvpDayta)に書き込む
			...
			*wvpDayta = worldViewProjectionMatrix;
			...
			// ==========================================
			// 4. ImGuiの描画（一番最前面に映すため最後に行う）
			// ==========================================
			// スライドの通り、描画用のDescriptorHeapを再設定する
			ID3D12DescriptorHeap* descriptorHeaps[] = { srvDescriptorHeap };
			commandList->SetDescriptorHeaps(1, descriptorHeaps);

			// レンダーターゲットからPresentに遷移させる前（ここ）で描画コマンドを積む
